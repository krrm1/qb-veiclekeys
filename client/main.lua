...
local function LockpickDoor(isAdvanced)
    ...
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if #(pos - vehpos) < 2.5 and not IsPedInAnyVehicle(ped) then
            local vehLockStatus = GetVehicleDoorLockStatus(vehicle)
            if (vehLockStatus > 0) then
                usingAdvanced = isAdvanced
                TriggerEvent('start:lockpicking:door')
            end
            else
                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then 
                    TriggerEvent('start:lockpicking:vehicle')
                  end
        end
    end
end
...
RegisterNetEvent('start:lockpicking:door', function()
	...
	if success then
        ...
        QBCore.Functions.Notify("lock picking door succeeded!")
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
	else
        PoliceCall()
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        QBCore.Functions.Notify("lock picking door failed!", "error")
	end
end)

RegisterNetEvent('start:lockpicking:vehicle', function()
	local time = math.random(7,10)
	...
	if success then
        ...
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        TriggerEvent('vehiclekeys:client:SetOwner', QBCore.Functions.GetPlate(vehicle))
        QBCore.Functions.Notify("lock picking vehicle succeeded!")
	else
        SetVehicleEngineOn(veh, false, false, true)
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        QBCore.Functions.Notify("lock picking vehicle failed!", "error")
	end
end)
